...
def _candidate_score(
    ...
    if area < image_area * min_area_ratio or area > image_area * 0.95:
        return -1.0
    ...
    return (area / image_area) - border_penalty
...
def _edge_maps(
    ...
    for lower, upper in threshold_pairs:
        ...
        maps.append((f"Canny {lower}-{upper}", closed))
    ...
    adaptive_edges = cv2.morphologyEx(adaptive_edges, cv2.MORPH_CLOSE, kernel, iterations=close_iterations)
    maps.append(("adaptive threshold edges", adaptive_edges))
    return maps
...
def auto_detect_quadrilateral(
    ...
    for method, edge_map in _edge_maps(
        ...
        for contour in contours[:80]:
            ...
            for epsilon_ratio in (0.01, 0.02, 0.03, 0.04, 0.06):
                ...
                if score > best_score:
                    best_score = score
                    best_points = points
                    best_method = f"{method}, polygon approx {epsilon_ratio:.2f}"
                    best_edges = edge_map

            rect = cv2.minAreaRect(contour)
            box = cv2.boxPoints(rect).astype(np.float32)
            points = order_points(box)
            score = _candidate_score(points, image_rgb.shape, min_area_ratio) * 0.8
            if score > best_score:
                best_score = score
                best_points = points
                best_method = f"{method}, min-area rectangle fallback"
                best_edges = edge_map
...
def enhance_scan(image_rgb: np.ndarray) -> np.ndarray:
    gray = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2GRAY)
